...
kwargs = {"examples": train_examples,
          ...
          "max_num_length": conf.max_num_length,
          "is_training": True,
          "op_list": op_list,
          "op_list_size": len(op_list),
          "const_list": const_list,
          "const_list_size": len(const_list),
          "num_tokenizer" : number_char_word2index,
          "verbose": True}
...
def train():
    ...
    model = Bert_model(num_decoder_layers=conf.num_decoder_layers,
                       ...
                       op_list=op_list,
                       const_list=const_list,
                       num_char_length = len(number_char_word2index),
                       num_emb_dim = conf.num_emb_dim)

    model = nn.DataParallel(model)
    model.to(conf.device)

    #######################
    # Be sure to remove this line if want to train from scratch!!!
    ...
    for _ in range(conf.epoch):
        ...
        for x in train_iterator:
            # print("num_char_ids.size(): ", torch.tensor(x['num_char_ids']).size())
            ...
            num_char_ids = torch.tensor(x['num_char_ids']).to(conf.device)  # [batch, seq_length, 10]
            
            num_char_mask = torch.tensor(x['num_char_mask']).to(conf.device)  # [batch, seq_length]
            number_mask = torch.tensor(x['number_mask']).to(conf.device)  # [batch, seq_length]
            model.zero_grad()
            optimizer.zero_grad()

            this_logits = model(True, input_ids, input_mask, segment_ids,
                                option_mask, program_ids, program_mask,
                                num_char_ids, number_mask, num_char_mask, device=conf.device)
            ...
            if k > 1 and k % conf.report == 0:
                ...
                if k // conf.report >= 1:
                    ...
                    print("Current best exe accuracy is ", best_exa_acc)
                    exa_acc = evaluate(
                        valid_examples, valid_features, model, results_path_cnt, 'valid')
                    if exa_acc > best_exa_acc:
                        best_exa_acc = exa_acc
                        print("Current exact accuracy is the best. Save the model!")
                        # save model
...
def evaluate(data_ori, data, model, ksave_dir, mode='valid'):
    ...
    with torch.no_grad():
        for x in tqdm(data_iterator):
            ...
            num_char_ids = x['num_char_ids']
            num_char_mask = x['num_char_mask']
            number_mask = x['number_mask']
            
            ori_len = len(input_ids)
            for each_item in [input_ids, input_mask, segment_ids, program_ids, program_mask, option_mask,  number_mask]:
                if ori_len < conf.batch_size_test:
                    each_len = len(each_item[0])
                    pad_x = [0] * each_len
                    each_item += [pad_x] * (conf.batch_size_test - ori_len)

            for each_item in [num_char_ids, num_char_mask]:
                if ori_len < conf.batch_size_test:
                    each_size = torch.tensor(each_item).size()[1:]
                    pad_x = [ [0] * each_size[1] ] * each_size[0]
                    each_item += [pad_x] * (conf.batch_size_test - ori_len)
            ...
            num_char_ids = torch.tensor(num_char_ids).to(conf.device)  # [batch, seq_length, 10]
            num_char_mask = torch.tensor(num_char_mask).to(conf.device)  # [batch, seq_length]
            number_mask = torch.tensor(number_mask).to(conf.device)

            # print(" input_mask: ", input_mask.size())
            # print("segment_ids : ", segment_ids.size())
            # print(" program_mask: ", program_mask.size())
            # print("option_mask : ", option_mask.size())
            # print("num_char_ids : ", num_char_ids.size())
            # print("num_char_mask : ", num_char_mask.size())
            # print("number_mask : ", number_mask.size())

            logits = model(False, input_ids, input_mask,
                           segment_ids, option_mask, program_ids, program_mask,
                           num_char_ids, number_mask, num_char_mask, device=conf.device)
