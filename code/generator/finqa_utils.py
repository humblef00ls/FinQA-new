...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.num_char_mask = num_char_mask
...
def convert_num_token_to_ids(max_num_length, numbers, num_tokenizer, number_mask):
    ...
    if max_length < max_num_length:
        max_num_length = max_length
    """
    num_char_ids_return = [[value] * max_num_length for value in number_mask]  # shape: [max_num_length, len(numbers)]
    num_char_mask_return = [[value] * max_num_length for value in number_mask]
    ...
    for number in numbers:
        ...
        mask = [1] * len(number_list) + padding
        ...
        num_char_mask.append(mask)
        # print("number_list: ", number_list)
        # print("num_char_ids: ", num_char_ids)
        # print("num_char_mask: ", num_char_mask)
    count = 0
    for idx in range(len(num_char_ids_return)):
        if num_char_ids_return[idx][0] == 1:
            num_char_ids_return[idx] = num_char_ids[count]
            num_char_mask_return[idx] = num_char_mask[count]
            count += 1

    return num_char_ids_return, num_char_mask_return
...
def convert_single_mathqa_example(example, is_training, tokenizer, max_seq_length,
    ...
    num_char_ids, num_char_mask = convert_num_token_to_ids(max_num_length, example.numbers, num_tokenizer, number_mask)
    # print("num_char_ids: ", torch.Tensor(num_char_ids).size())
    # print("num_char_mask: ", torch.Tensor(num_char_mask).size())
    # print("num_mask: ", torch.Tensor(number_mask).size())
